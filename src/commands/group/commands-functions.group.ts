...
export async function mtaCommand(client: WASocket, botInfo: Bot, message: Message, group: Group) {
    const groupCommands = commandsGroup(botInfo);
    const generalMessages = getGeneralMessages(botInfo);
    
    let userMessage: string = message.args.length ? message.text_command.trim() : "*ATENÇÃO!*";
    
    // Criar a lista de menções sem incluí-las na mensagem
    let mentionList: string[] = group.participants.map(participant => `@${participant.split('@')[0]}`);
    
    await Whatsapp.replyWithMentions(client, message.chat_id, userMessage, group.participants, message.wa_message, { expiration: message.expiration });
}
